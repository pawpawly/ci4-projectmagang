...
<script>
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
        if (deleteUsername) {
            fetch("<?= site_url('superadmin/user-management/delete/') ?>" + deleteUsername, {
                method: 'DELETE'
            })
            .then(response => {
                if (response.ok) {
                    document.getElementById('user-' + deleteUsername).remove();
                    showNotification('Pengguna berhasil dihapus.');
                } else {
                    showNotification('Gagal menghapus pengguna.');
                }
            })
            .catch(() => showNotification('Terjadi kesalahan saat menghapus pengguna.'));
            cancelDelete();
        }
    });
