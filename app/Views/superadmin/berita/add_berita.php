...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md mt-10">
    ...
    <form id="tambahBeritaForm" action="<?= site_url('berita/save') ?>" method="POST" enctype="multipart/form-data">
        <div class="mb-4">
        ...
        <div class="mt-6 flex justify-end space-x-4">
            ...
            <button type="button" onclick="confirmAddBerita()" 
...
<!-- Modal Konfirmasi -->
<div id="confirmAddBeritaModal" class="hidden fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg shadow-lg">
        <h2 class="text-lg font-bold mb-4">Konfirmasi Tambah Berita</h2>
        <p>Apakah Anda yakin ingin menambahkan berita ini?</p>
        <div class="mt-6 flex justify-end space-x-4">
            <button onclick="cancelAddBerita()" 
                    class="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600">Batal</button>
            <button onclick="submitBeritaForm()" 
                    class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">Simpan</button>
        </div>
    </div>
</div>
...
<script>
    function confirmAddBerita() {
        const modal = document.getElementById('confirmAddBeritaModal');
        modal.classList.remove('hidden');
    }

    function cancelAddBerita() {
        const modal = document.getElementById('confirmAddBeritaModal');
        modal.classList.add('hidden');
    }

    function submitBeritaForm() {
        document.getElementById('tambahBeritaForm').submit();
    }
</script>
